...
int main (int argc, char* argv[]) {
	...
	printf("Size of filesystem in fragment size units:  %u\n", st_vfs.f_blocks);
	printf("Amount of free blocks:                      %u\n", st_vfs.f_bfree);
	printf("Amount of free blocks for non-root:         %u\n", st_vfs.f_bavail);
	printf("Amount of inodes:                           %u\n", st_vfs.f_files);
	printf("Amount of free inodes:                      %u\n", st_vfs.f_ffree);
	printf("Amount of free inodes for non-root:         %u\n", st_vfs.f_favail);
	...
	if ((st_vfs.f_favail < 1) || ((long int)(st_vfs.f_bavail * st_vfs.f_bsize) < sb.st_size)) /* checking if there is inode numbers or        */
